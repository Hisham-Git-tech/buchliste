...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            flash(_('Ungültiger Benutzername oder Passwort'))
    ...
    return render_template('auth/login.html', title=_('Anmelden'), form=form)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(_('Glückwunsch, du bist jetzt registriert!'))
        return redirect(url_for('auth.login'))
